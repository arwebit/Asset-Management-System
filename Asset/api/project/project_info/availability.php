...
if (($status_typeErr == "") && ($projectErr == "") && ($tokenErr == "")) {
    $credential[] = array("Createuser" => $create_user, "Project_info_id" => $project_info_id, "Status" => $status,
        "Current_date" => $today_date_time);
    $updateStatus = $projectInfoObj->availProjectInfo(json_encode($credential));
    ...
    $dataErrs[] = array("TokenErr" => $tokenErr, "StatusErr" => $status_typeErr, "ProjectInfoErr" => $project_infoErr);
    $response['error'] = true;

...
foreach ($data_cred as $data_credVal) {
    ...
    $gatepass_link_id = $data_credVal->gatepass_link_id;
...
if (empty($gatepass_link_id)) {
    $gatepass_link_idErr = "Required";
}
if (empty($reference)) {
...
if (($tokenErr == "") && ($gatepass_link_idErr == "") && ($referenceErr == "") && ($reportdtErr == "") && ($supervisor_nameErr == "") && ($work_doneErr == "") && ($work_statusErr == "") && ($work_done_byErr == "") && ($detailsErr == "") && ($emp_remarkErr == "") && ($material_shortageErr == "")&& ($pending_workErr == "")) {
    $credential[] = array("GatepassLinkID" => $gatepass_link_id, "SafetyCL" => $safety_clearence, "HrCL" => $hr_clearence, "ReportDateTime" => $reportdt,
    ...
    $insertStatus = $reportObj->createReport(json_encode($credential));

    if ($insertStatus > 0) {
        $dataSucc[] = array("SuccessMsg" => "Successfully submitted");
    ...
    $dataErrs[] = array("TokenErr" => $tokenErr, "ProjectErr" => $project_idErr, "ReferenceErr" => $referenceErr, "ReportDateErr" => $reportdtErr, "SupervisorErr" => $supervisor_nameErr,
        ...
        "EmpRemarkErr" => $emp_remarkErr, "PendingWorkErr" => $pending_workErr);
    $response['error'] = true;

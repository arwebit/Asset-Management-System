...
foreach ($getReportData as $getReportDataVal) {
    ...
    $role_id = trim($getReportDataVal->role_id);
}
if (empty($token)) {
    $tokenErr = "Required";
} else {
...
if (($tokenErr == "")&&($role_idErr == "")) {
    $searchCred[] = array("CreateUnder" => $create_under, "LoginUser" => $login_user, "RoleID" => $role_id);
    $retVal = $reportObj->getReportDetails(json_encode($searchCred));
    foreach ($retVal as $retVals) {
        $getCountRecord = $retVals['Record'];
    ...
    $dataErrs[] = array("TokenErr" => $tokenErr, "RoleIDErr" => $role_idErr);
    $response['error'] = true;

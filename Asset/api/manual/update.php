...
foreach ($getManualData as $getManualDataVal) {
    $modify_user = trim($getManualDataVal->session_user);
...
if (($project_idErr == "") && ($manual_titleErr == "") && ($manual_descrErr == "") && ($tokenErr == "")) {
    $credential[] = array("ProjectID" => $project_id, "ManualTitle" => $manual_title, "ManualDescr" => $manual_descr,
        "Modifyuser" => $modify_user, "Updatemanualid" => $manual_id, "Current_date" => $today_date_time);
    $updateStatus = $manualObj->updateManual(json_encode($credential));

    if ($updateStatus > 0) {
        $dataSucc[] = array("SuccessMsg" => "Successfully updated manual");

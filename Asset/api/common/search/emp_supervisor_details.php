...
if (($tokenErr == "") && ($employee_supervisorErr == "") && ($user_roleErr == "")) {
    $searchCred[] = array("CreateUnder" => $create_under, "Employee_Supervisor" => $employee_supervisor, "CurrentDate" => $today_date_time,
        "LoginUser" => $login_user, "UserRole" => $user_role);
    $retVal = $searchObj->getEmployeeSupervisorStatus(json_encode($searchCred));
    foreach ($retVal as $retVals) {
        $getCountRecord = $retVals['Record'];
    ...
    $dataErrs[] = array("TokenErr" => $tokenErr, "UserRoleErr" => $user_roleErr,
        "Employee_SupervisorErr" => $employee_supervisorErr);
    $response['error'] = true;

...
if ($_SESSION['asset_member'] && $_SESSION['asset_token']) {
    ...
    if ($_REQUEST['daily_report_id']) {
        ...
        if (isset($_REQUEST['sup_remark_btn'])) {
            $supervisor_remark = trim($_REQUEST['supervisor_remark']);
            $status = trim($_REQUEST['action_status']);
            $supervisor_remark_data[] = array("session_token" => $login_token, "session_user" => $login_user, "report_id" => $daily_report_id,
                "remark" => $supervisor_remark, "report_status" => $status);
            $sr_data = json_encode($supervisor_remark_data);
            $save_sup_remark_getData = json_decode(callAPI($sr_data, $sup_remark_daily_report_api));
            $save_sup_remark_error = $save_sup_remark_getData->error;
            $save_sup_remark_message = $save_sup_remark_getData->message;
            $save_sup_remark_data = $save_sup_remark_getData->data;

            if ($save_sup_remark_error == true) {
                if ($save_sup_remark_message == "Server error") {
                    foreach ($save_sup_remark_data as $dataVal) {
                        $errMsg = $dataVal->ErrorMessage;
                    }
                } else {
                    foreach ($save_sup_remark_data as $dataVal) {
                        $tokenErr = $dataVal->TokenErr;
                        $reportErr = $dataVal->ReportErr;
                        $supervisor_remarkErr = $dataVal->RemarkErr;
                        $statusErr = $dataVal->StatusErr;
                    }
                }
            } else {
                                                                    ...
                                                                    <ul>
                                                                        ...
                                                                        foreach ($report_media_data as $report_media_dataVal) {
                                                                            ...
                                                                            if (($media_type == "image/jpg") || ($media_type == "image/jpeg") || ($media_type == "image/png")) {
                                                                                ?>
                                                                                <li><img src="../api/employee/reports/<?php echo $media_path; ?>"/></li>
                                                                                <?php
                                                                            } else {
                                                                                ?>
                                                                                <li>
                                                                                    <video controls width="500" height="300">
                                                                                        <source src="../api/employee/reports/<?php echo $media_path; ?>" type="<?php echo $media_type; ?>">
                                                                                    </video>
                                                                                </li>
                                                                                <?php
                                                                            }
                                                    ...
                                                    <div class="row">
                                                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                                            <div class="form-group">
                                                                ...
                                                                <b class="text-danger"><?php echo $statusErr; ?></b>
                                                            </div>
                                                        </div>
                                                        <div class="col-lg-6">
                                                            <div class="form-group">
                                                                <label for="supervisor_remark">Supervisor remark</label> <b class="text-danger"> </b>
                                                                <textarea required="required" class="form-control" name="supervisor_remark" id="remark">
                                                                    <?php echo $sup_remark; ?>
                                                                </textarea>
                                                                <b class="text-danger"><?php echo $supervisor_remarkErr; ?></b>
                                                            </div>
                                                        </div>
                                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                            <button type="submit" class="btn btn-primary" name="sup_remark_btn" value="1" id="sup_remark_btn">
                                                                SUBMIT
                                                            </button>
